...
- minification is done with rolldown (typically Vite 8 or higher)
...
ac=function(e){return e[e.NumberEnumItem=123]=`NumberEnumItem`,e.StringEnumItem=`ABC`,e}({})
...
async function enumParser(path: string) {
    ...
    while (notDone) {
        if (mode === modes.new) {
            ...
            if (isIdentChar(nextChar)) {
                ...
                readIdx++;
            } else if (nextChar === '=' && inputData.slice(readIdx + 1, readIdx + 10) === 'function(' && currPRoot !== '') {
                // have found the end of a valid identifier, and the characters after it match our pattern
            ...
            if (isIdentChar(nextChar)) {
                ...
                readIdx++;
            } else if (nextChar === ')' && inputData.slice(readIdx + 1, readIdx + 9) === '{return ' && currIRoot !== '') {
                // have found the end fo a valid identifier, and the following chars after match our pattern
            ...
            if (gutsEnd === -1) {
                ...
                // matching test: starts with one of the valid prefixes, ends with slanty-quote, contains an equals.
    ...
    enumsFound.forEach(enumDef => {
        ...
        const rawStart = fullDef.indexOf(`){return `) + 9;
        ...
        const gutNuggets = rawGuts.split(',').map(s => {
            // this will be innerRoot.enumItemName=enumValue
            ...
            if (s.startsWith(numFormat)) {
                ...
                // string format, take entire nugget. replace backticks with classic quotes.
                newEnumDef = s.replaceAll('`', '"');
            }
